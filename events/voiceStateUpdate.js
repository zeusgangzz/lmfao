...
module.exports = (client, oldState, newState) => {
    ...
    const embed = new client.embed()
        ...
        .setFooter(`Astral Networks | Made by the devs`, newState.guild.iconURL({ dynamic: true }))
    ...
    else if (oldState.selfDeaf !== newState.selfDeaf && newState.selfDeaf) log.send(embed.setDescription(`**${newState.member.user.tag}** has deafened!`))
    else if (oldState.selfDeaf !== newState.selfDeaf && !newState.selfDeaf) log.send(embed.setDescription(`**${newState.member.user.tag}** has undeafened!`))

    else if (oldState.selfMute !== newState.selfMute && newState.selfMute) log.send(embed.setDescription(`**${newState.member.user.tag}** has muted!`))
    else if (oldState.selfMute !== newState.selfMute && !newState.selfMute) log.send(embed.setDescription(`**${newState.member.user.tag}** has unmuted!`))

    else if (oldState.channelID !== newState.channelID && newState.channelID && !oldState.channelID) log.send(embed.setDescription(`**${newState.member.user.tag}** joined a voice channel!\n**Joined a channel:** ${newState.channel}`))
    else if (oldState.channelID !== newState.channelID && oldState.channelID && !newState.channelID) log.send(embed.setDescription(`**${newState.member.user.tag}** left a voice channel!\n**Left a channel:** ${oldState.channel}`))
